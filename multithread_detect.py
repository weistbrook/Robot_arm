...
from utils.general import non_max_suppression, scale_coords  # YOLOv5工具函数：非极大值抑制、坐标缩放
from utils.datasets import letterbox  # YOLOv5工具函数：图像等比例缩放（带黑边填充）
...
# 加载本地训练的YOLOv5模型（确保权重文件路径正确）
model = torch.load(
    '/home/jetson/yolov5/runs/train/exp15/weights/best.pt',  # 模型权重路径
    map_location=device  # 加载到指定设备（GPU/CPU）
)['model'].float().fuse().eval()  # 转换为float类型，融合层优化，设置为评估模式
names = model.names  # 模型类别名称列表（如['valve']）
img_size = 640  # YOLOv5输入图像尺寸（640x640）
...
def image_callback(msg):
    ...
    try:
        ...
        # YOLOv5图像预处理：等比例缩放至640x640（带黑边填充）
        img = letterbox(frame, img_size, stride=32)[0]
        # 转换格式：BGR→RGB（YOLO模型输入为RGB），HWC→CHW（通道优先）
        img = img[:, :, ::-1].transpose(2, 0, 1)
        img = np.ascontiguousarray(img)  # 确保内存连续，提升PyTorch处理速度

        # 转换为PyTorch张量：移动到指定设备，归一化（0-255→0-1）
        img_tensor = torch.from_numpy(img).to(device).float() / 255.0
        img_tensor = img_tensor.unsqueeze(0)  # 添加批次维度（batch=1）

        # 执行YOLOv5推理（关闭梯度计算，提升速度）
        with torch.no_grad():
            pred = model(img_tensor)[0]  # 获取模型输出
        # 非极大值抑制（NMS）：过滤重叠检测框，保留高置信度结果
        dets = non_max_suppression(pred, conf_thres=0.65, iou_thres=0.45)
        # conf_thres=0.65：置信度阈值，低于此值的框被过滤；iou_thres=0.45：IOU阈值，控制重叠过滤
        ...
        for det in dets:
            ...
            elif xyz_small is not None and s_cls == 1 and 1000*xyz_small[2]<10 and xyz is None:
...
def main():
    ...
    rospy.init_node("ros_yolov5_node", anonymous=True)
