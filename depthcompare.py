...
file_path = 'paperneed/depth_compare.xlsx'
try:
...
for x, y in zip(df['真实值'], df['差值绝对值']):
    plt.text(x, y + 0.01, f'{y:.2f}', ha='center', va='bottom', fontsize=9, fontname='Times New Roman')

# 设置坐标轴和标题
plt.xlabel('True Value(cm)', fontsize=12, fontweight='bold', fontname='Times New Roman')
plt.ylabel('Absolute Difference(cm)', fontsize=12, fontweight='bold', fontname='Times New Roman')
...
plt.savefig('real vs experimental.pdf', format='pdf', bbox_inches='tight')
print("\n矢量图已保存为：real vs experimental.pdf")
plt.show()
