...
SAVE_DIR = os.path.join(os.getcwd(), "saved_frames")
...
def save_current_frame(frame):
    ...
    rospy.loginfo(f"Saved frame to: {save_path}")
...
def image_callback(msg):
    ...
    try:
        ...
        if results:
            ...
            if boxes is not None and len(boxes) > 0:
                ...
                if len(valve_det) > 0:
                    ...
                    valve_xyz = pixel_to_3d(valve_center_x, valve_center_y, depth_image, camera_info)
        ...
        height, width = frame.shape[:2]  # 获取图像宽高
        center_x, center_y = width // 2, height // 2  # 计算中心坐标
        cv2.circle(frame, (center_x, center_y), 5, (255, 0, 0), -1)  # 画实心蓝点
